    ...
    function scSave(loc){
        ...
        scRpc('app_metrics_save_secure',{p_location:loc,p_date:date,p_payload:payload},function(){ scLoad(); });
    }
    ...
    function fhFinanceRows(f){ var model=f.financial_model==='100_return_payout'?'100% return &mdash; organizer returns all; Caliche\'s pays org 50%':'50/50 &mdash; organizer keeps 50%, returns Caliche\'s 50%'; return fhRow('Model',model)+fhRow('Face value',fhMoney(f.face_value))+fhRow('Gross sold',f.gross_sold!=null?fhMoney(f.gross_sold):'&mdash;')+fhRow('Caliche\'s share',f.caliches_share!=null?fhMoney(f.caliches_share):'&mdash;')+fhRow('Org proceeds',f.org_share!=null?fhMoney(f.org_share):'&mdash;')+fhRow('Payout to org',f.payout_amount!=null?fhMoney(f.payout_amount):'&mdash;')+fhRow('Received',f.amount_received!=null?fhMoney(f.amount_received):'&mdash;')+fhRow('Variance',f.variance!=null?fhMoney(f.variance):'&mdash;')+fhRow('Receipt #',f.receipt_number); }
    ...
    function groupPollHtml(poll, mine){
        var total=poll.total||0, opts=poll.options||[], myVote=poll.my_vote;
    ...
    function pollAddOption(){ var box=document.getElementById('pollOpts'); if(!box) return; var n=box.querySelectorAll('.pollOpt').length; if(n>=6) return; var inp=document.createElement('input'); inp.className='pollOpt'; inp.placeholder='Option '+(n+1); inp.style.cssText='width:100%;box-sizing:border-box;padding:8px;border:1px solid #d6deea;border-radius:8px;font-size:13px;margin-bottom:6px;'; box.appendChild(inp); }
    function groupPollCreate(){
        var q=((document.getElementById('pollQ')||{}).value||'').trim();
        var opts=[].slice.call(document.querySelectorAll('#pollOpts .pollOpt')).map(function(i){return (i.value||'').trim();}).filter(Boolean);
        if(!q){ alert('Add a question.'); return; }
        if(opts.length<2){ alert('Add at least two options.'); return; }
        withPin(function(pin){ supabaseClient.rpc('app_poll_create',{p_username:currentUser.username,p_password:pin,p_group_id:groupWith,p_question:q,p_options:opts}).then(function(r){ if(r.error){ if(r.error.code==='42501') sessionPin=null; alert('Error: '+r.error.message); return; } var o=document.getElementById('gPollOv'); if(o)o.remove(); renderGroup(); }); });
    }
    function groupVote(pollId, optId){ withPin(function(pin){ supabaseClient.rpc('app_poll_vote',{p_username:currentUser.username,p_password:pin,p_poll_id:pollId,p_option_id:optId}).then(function(r){ if(r.error){ if(r.error.code==='42501') sessionPin=null; alert('Error: '+r.error.message); return; } renderGroup(); }); }); }
