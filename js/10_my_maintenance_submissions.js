    ...
    function taxRatesRemove(city){ if(!confirm('Remove '+city+'?')) return; withPin(function(pin){ var key='city_'+String(city).toLowerCase().replace(/[^a-z0-9]+/g,'_'); supabaseClient.rpc('app_settings_delete',{p_username:currentUser.username,p_password:pin,p_key:key}).then(function(r){ if(r.error){ alert(r.error.message||'Could not remove.'); return; } delete HUB_TAX_RATES[city]; taxRatesRender(); hubLoadTaxRates(); }).catch(function(){ alert('Could not remove.'); }); }); }
